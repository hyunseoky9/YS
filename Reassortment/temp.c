...
int main(int argc, char *argv[])
{
	...
	double* factorpre = (double*) malloc(sizeof(double)*(2*kmax)); // probability of getting n mutations organized in an array
	double factor[2*kmax]; // sum of factorpre factor[3] is a sum of factorpre[0] through factorpre[3] 
	double**** pop;
	...
	double** mutmat = (double**) malloc(sizeof(double*)*pow((kmax+1),2)); // mutation matrix
	for(i=0; i<=(int)pow((kmax+1),2); i++)
	{
		mutmat[i] = (double*) malloc(sizeof(double)*pow((kmax+1),2));
	}
	...
	/*
	int coor1, coor2;
	for (i=0; i<pow(kmax+1,2); i++)
	{
		// convert matrix coordinate to j,k coordinate
		coor1 = floor((double)i/(kmax+1)); // j
		coor2 = i - (coor1*(kmax+1)); // k
		printf("(%d,%d) ",coor1,coor2);
		for(j=0; j<pow(kmax+1,2); j++)
		{
			printf("%.6f ",mutmat[i][j]);
		}
		printf("\n");
	}*/
