...
class Virus1():
    ...
    def mutate(self,mu):
        ...
        mu = mutation rate
        """
        mut_num = np.random.binomial(L, mu) # number of mutation
        self.k += mut_num
...
class Virus2():
    ...
    def mutate(self,mu):
        ...
        """
        mut_num = np.random.binomial(L, mu) # number of mutation
        split_pt = int(np.ceil(np.random.uniform(0,1)*mut_num)) # decide how much of mutation goes to seg1 and seg2
        self.k1 += split_pt
