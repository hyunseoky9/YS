...
int main(int argc, char *argv[])
{
	...
	sprintf(filename,"%s/m1.1.1s_%d,%d,%d,%.3f,%d,%d,%.5f,%d,%.2f,%.2f,%d,%d(0).csv",dest2,timestep,krecord,rep,s,N0,K,u,gen_num,c,r,kmax,host_num);
	int filenum = 0;

	while ( access(filename, F_OK) != -1) 
	{
		filenum += 1;
		sprintf(filename,"%s/m1.1.1s_%d,%d,%d,%.3f,%d,%d,%.5f,%d,%.2f,%.2f,%d,%d(%d).csv",dest2,timestep,krecord,rep,s,N0,K,u,gen_num,c,r,kmax,host_num,filenum);
	}
	FILE* fPointer;
...
void mutate(double**** pop, int* curpop, double u, int kmax, int host_num, double factor[])
{
	...
	*/
	...
	double equate;
	int select;
	int mutcap = 5; // maximum number of mutation accumulation possible in a step
	int cap;
	for (i=0; i<host_num; i++)
	{
		for (j=0; j<=kmax; j++)
